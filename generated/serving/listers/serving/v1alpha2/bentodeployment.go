...
type BentoDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.BentoDeployment, err error)
...
// List lists all BentoDeployments in the indexer.
func (s *bentoDeploymentLister) List(selector labels.Selector) (ret []*v1alpha2.BentoDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.BentoDeployment))
	})
	return ret, err
}
...
type BentoDeploymentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.BentoDeployment, error)
...
// List lists all BentoDeployments in the indexer for a given namespace.
func (s bentoDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.BentoDeployment, err error) {
...
// Get retrieves the BentoDeployment from the indexer for a given namespace and name.
func (s bentoDeploymentNamespaceLister) Get(name string) (*v1alpha2.BentoDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("bentodeployment"), name)
	}
	return obj.(*v1alpha2.BentoDeployment), nil
}
